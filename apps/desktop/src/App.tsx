...
function defaultTaskForm(): TaskForm {
  const due = defaultTaskDueFields();
  return {
    title: "",
    notes: "",
    dueDate: due.dueDate,
...
function normalizeTaskDueDateInput(value: string) {
  ...
  const localizedDate = /^(\d{1,2})[/-](\d{1,2})[/-](\d{4})$/.exec(trimmed);
  if (localizedDate) {
...
function taskFormFromTask(task: BackendTask): TaskForm {
  ...
  const defaultDue = defaultTaskDueFields();
  const dueDate = validDueAtDate
    ? formatLocalDateInput(validDueAtDate)
    : normalizeTaskDueDateInput(task.dueDate ?? "") || defaultDue.dueDate;
  const dueTime = validDueAtDate
    ? localTimeInputFromDate(validDueAtDate)
    : defaultDue.dueTime;
...
function MyTasksView({
  ...
  const generateTaskReport = () => {
    ...
    window.setTimeout(() => taskReportOutputRef.current?.scrollIntoView({ block: "nearest", behavior: "smooth" }), 0);
  };
