...
describe("DayTrail command center", () => {
  ...
  it("manages overall tasks and reminders from My Tasks", async () => {
    ...
    const taskAfterEdit =
      screen.queryByText(/renew vendor agreement/i) ?? screen.getByText(/renew vendor contract/i);
    const taskRowAfterEdit = taskAfterEdit.closest(".task-row");
    expect(taskRowAfterEdit).not.toBeNull();
    ...
    const invoke = vi.fn(async (command: string, args?: Record<string, unknown>) => {
      ...
      if (command === "update_settings") {
        ...
        if (patch.taskRetentionDays !== undefined) {
          expect(args).toEqual({ patch: { taskRetentionDays: 90 } });
          return {
            browserBridgeEnabled: true,
            excludedDomains: [],
            aiProvider: "Ollama Local",
